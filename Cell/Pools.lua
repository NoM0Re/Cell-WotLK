...
local type = type
local Mixin = F.Mixin
local CreateFromMixins = F.CreateFromMixins
local nop = F.noop
...
local function ForbiddenFramePoolFactory(framePool)
	local createFrameFunc = CreateForbiddenFrame or CreateFrame;
	return createFrameFunc(framePool.frameType, nil, framePool.parent, framePool.frameTemplate);
end
	...
	if forbidden then
		...
		if frameInitFunc ~= nil then
			creationFunc = function(framePool)
				local createFrameFunc = CreateForbiddenFrame or CreateFrame;
				local frame = createFrameFunc(framePool.frameType, nil, framePool.parent, framePool.frameTemplate);
				frameInitFunc(frame);
				return frame;
			end
		end
